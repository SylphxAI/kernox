...
//! 2% regression from run-to-run noise, so the gate deliberately aggregates
//! repeated invocations. `bench-budget-record` deletes the recorded
//! `steady-state-call` results and runs the benchmark `N` times (default 5)
//! with `--save-baseline run1..runN`. The gate then reads both benchmarks'
//! `run<i>/estimates.json` for every `i` and enforces the per-side minimum
//! (the fastest observed run on each side), the standard estimator for
//! one-sided loading noise:
//!
//! ```text
//! direct_min = min(direct_point_1..direct_point_N)
//! kernox_min = min(kernox_point_1..kernox_point_N)
//! statistic = (kernox_min - direct_min) / direct_min
//! PASS iff statistic <= 0.02      # exactly 2% passes
//! ```
//!
//! Loading can only add time, so a loaded run cannot pull either side below
//! its clean execution time; a genuine regression raises every Kernox run and
//! therefore the minimum. The per-run deltas, their median, and their spread
//! are printed for inspection but do not decide the gate.
//!
//! Same-run binding: the gate requires the invocation start epoch
//! (`--not-before`, written before `bench-budget-record` runs) and the strict
//! interleaved file order `direct1 < kernox1 < direct2 < kernox2 < ... ` on
//! the `estimates.json` modification times. Every one of the `2 * N` files is
//! required. Files from an earlier invocation, mixed fresh/stale sides, or
//! reordered recordings therefore fail closed instead of being silently
//! averaged in.
//!
...
/// Declared acceptance budget for the median steady-state point-estimate delta.
pub(crate) const STEADY_STATE_BUDGET: f64 = 0.02;

/// Enforced number of recorded benchmark invocations in the scheduled lane.
pub(crate) const DEFAULT_RUNS: usize = 5;

/// Fewer runs cannot make the median robust, so the gate refuses them.
...
/// Aggregates the recorded runs: refuses stale, mixed, or out-of-order
/// recordings, then enforces the per-side minimum point-estimate delta budget.
pub(crate) fn evaluate_runs(
    ...
    let statistic_delta = (kernox_min - direct_min) / direct_min;
    if !statistic_delta.is_finite() {
    ...
    Ok(RunsOutcome {
        ...
        pass: statistic_delta <= STEADY_STATE_BUDGET,
        direct_min,
        kernox_min,
...
fn check_runs(runs: usize) -> Result<(), String> {
    if runs < MIN_RUNS {
        return Err(format!(
            "at least {MIN_RUNS} recorded runs are required so the median is meaningful; got {runs}"
        ));
    }
    Ok(())
}
...
/// Runs the gate: reads every recorded run, proves the recording is one fresh
/// ordered invocation, prints the machine-readable report, and fails when the
/// median delta exceeds the declared budget or the evidence is unusable.
pub(crate) fn run(options: &GateOptions) -> Result<(), String> {
    ...
    if outcome.pass {
        ...
        Err(format!(
            "min-side steady-state point-estimate delta {:.6} exceeds declared budget {:.6}",
            outcome.statistic_delta, STEADY_STATE_BUDGET
        ))
    }
}
...
fn print_report(criterion_dir: &Path, runs: usize, samples: &[RunSample], outcome: &RunsOutcome) {
    ...
    println!("bench-budget.statistic=min-side-delta");
    for (index, sample) in samples.iter().enumerate() {
        let run = index + 1;
    ...
    println!("bench-budget.delta={:.6}", outcome.statistic_delta);
...
mod tests {
    ...
    #[test]
    fn min_side_delta_exactly_two_percent_passes() {
        let samples = runs_with([0.02; 5]);
        let outcome = evaluate_runs(&samples, epoch(1_000)).expect("valid recording");
        assert!((outcome.statistic_delta - STEADY_STATE_BUDGET).abs() < f64::EPSILON);
        assert!(outcome.pass);
    }

    #[test]
    fn min_side_delta_above_two_percent_fails() {
        let samples = runs_with([0.0201; 5]);
        let outcome = evaluate_runs(&samples, epoch(1_000)).expect("valid recording");
        assert!(outcome.statistic_delta > STEADY_STATE_BUDGET);
        assert!(!outcome.pass);

        let genuine_regression = runs_with([0.10; 5]);
        let outcome = evaluate_runs(&genuine_regression, epoch(1_000)).expect("valid recording");
        assert!((outcome.statistic_delta - 0.10).abs() < f64::EPSILON);
        assert!(!outcome.pass);
    }

    #[test]
    fn min_side_statistic_ignores_loaded_runs() {
        ...
        assert!((outcome.statistic_delta - 0.01).abs() < 1e-12);
        assert!(outcome.pass);
    ...
    #[test]
    fn min_side_statistic_fails_on_a_consistent_regression() {
        // No loaded run can hide this: every Kernox run is at least 3% slow.
    ...
    fn gate_arguments_require_not_before_and_minimum_runs() {
        ...
        .expect_err("two runs are refused");
