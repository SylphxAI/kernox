...
| T-06 | Plugin task ignores cancellation, panics silently, or prevents shutdown | Host task scope, panic capture without payload retention, fail-closed admission, timer-backed bounded drain, format-safe named report, forced task abort, runtime-affine teardown failure, and readiness rejection when Tokio timers are unavailable | Paused-time cooperative/stubborn/panicking task fixtures, no-timer readiness and runtime-less teardown regressions, and invalid-label tests in `kernox-host-tokio` |
