...
| T-06 | Plugin task ignores cancellation, panics silently, or prevents shutdown | Host task scope, panic capture without payload retention, fail-closed admission, timer-backed bounded drain, format-safe named report, forced task abort, and readiness rejection when Tokio timers are unavailable | Paused-time cooperative/stubborn/panicking task fixtures, no-timer readiness regression, and invalid-label tests in `kernox-host-tokio` |
