...
| KR-COMPAT | Compatibility and extension ladder | live | KR-CAP, KR-HOST | Host capability negotiation fails closed before plugin initialization on missing, duplicate, or incompatible host properties. Composition and graph-report readers reject unsupported schema majors. Native dynamic libraries and WebAssembly Component plugins remain unadmitted until an accepted ABI/WIT, capability-grant, resource, and migration contract exists. `docs/compatibility.md` is the compatibility policy. Public-API comparison against published predecessors, when they exist, is evidence, not a substitute for those fail-closed readers. |
